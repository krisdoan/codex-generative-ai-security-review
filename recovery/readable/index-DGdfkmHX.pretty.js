...
function jN() {
  ...
  J.useEffect(() => {
    ...
    ge(x);
    try {
      const N = sessionStorage.getItem(Xf), R = N ? JSON.parse(N) : null;
      R != null && R.value && _e(R);
    } catch {
    }
...
Continue anyway?`)) return;
    ...
    if (!je.hasGeminiApiKey && !(Q != null && Q.value)) {
      Mt((x) => ({ ...x, error: "API key is required before generation. Ask an admin to set a permanent key (shared) or set a temporary key." }));
      return;
    }
    Mt({ isLoading: true, results: [], error: null }), zt("Preparing generation..."), ri.current = false, cl(0), ha("queued");
    try {
      const x = je.hasGeminiApiKey;
      let N = null, R = null, O = null, W = null;
      x ? (Ui && (Ge.environment === "Reference upload (default)" || Ge.environment === "Randomize") && (zt("Uploading environment reference…"), O = await fi(Ui, "env")), cn && (Ge.clothing.includes("1") || Ge.clothing === "Randomize") && (zt("Uploading fashion reference…"), W = await fi(cn, "fashion"))) : (Ui && (Ge.environment === "Reference upload (default)" || Ge.environment === "Randomize") && (zt("Analyzing environment reference..."), N = await KN(Ui, Q == null ? void 0 : Q.value)), cn && (Ge.clothing.includes("1") || Ge.clothing === "Randomize") && (zt("Analyzing fashion reference..."), R = await $N(cn, Q == null ? void 0 : Q.value)));
      const se = [];
      let de = 0;
    ...
    if (!S && !(Q != null && Q.value)) {
      Io("API key is required before editing.");
      return;
    }
    Io(null), pl(true);
    try {
      ...
      let N;
      if (S) {
        const Ce = Oe.mimeType || Zi(Oe.imageUrl) || "image/png", tt = `edit_source_${Date.now()}.${Nn(Ce)}`, Ct = Oe.imageUrl.startsWith("http") ? Oe.imageUrl : await Ha(Oe.imageUrl, tt), Tt = await Pe("/api/edit", { method: "POST", body: JSON.stringify({ imageUrl: Ct, instruction: x }) });
        if (!(Tt != null && Tt.ok)) throw new Error((Tt == null ? void 0 : Tt.error) || "Edit failed.");
        N = Tt.imageUrl;
      } else N = await ky(Oe.imageUrl, x, Q.value);
    ...
    const S = je.hasGeminiApiKey;
    if (!S && !(Q != null && Q.value)) {
      pn("API key is required before editing.");
      return;
    }
    if (et.kind !== "image") {
    ...
    try {
      ...
      let N;
      if (S) {
        const ee = await Pe("/api/edit", { method: "POST", body: JSON.stringify({ imageUrl: et.url, instruction: x }) });
    ...
    if (!s || s.role !== "admin") return;
    if (!Ve.trim()) {
      pe("API key cannot be empty.");
      return;
    }
    const S = { ownerEmail: s.email, value: Ve.trim(), scope: K, savedAt: (/* @__PURE__ */ new Date()).toISOString() };
    if (K === "temporary") {
      sessionStorage.setItem(Xf, JSON.stringify(S)), _e(S), I(""), Me(false), pe("API key saved (temporary).");
      return;
    }
    Pe("/api/config/defaults", { method: "POST", body: JSON.stringify({ geminiApiKey: S.value, geminiApiKeyScope: "permanent", geminiApiKeyOwnerEmail: S.ownerEmail, geminiApiKeySavedAt: S.savedAt }) }).then(() => Ht()).then(() => {
      I(""), Me(false), pe("API key saved (permanent, server).");
    }).catch((x) => pe((x == null ? void 0 : x.message) || "Failed to save API key."));
  }, pi = () => {
    !s || s.role !== "admin" || !window.confirm("Remove the stored API key? This will disable generation until an admin saves a new key.") || (sessionStorage.removeItem(Xf), _e(null), Pe("/api/config/defaults", { method: "POST", body: JSON.stringify({ removeGeminiApiKey: true }) }).then(() => Ht()).then(() => {
      Me(false), pe("API key removed.");
    }).catch(() => {
      Me(false), pe("API key removed locally (server remove failed).");
    }));
